...
function newValue() {
  ...
  var newData = "";
  data.map((idx) => {
    newData += `color: ${idx.color}, value: ${idx.value} <br />`;
  });

  if (x.innerHTML === "Welcome") {
    // const para = document.createElement("p");
    // para.innerHtml = "Exmaple of data";
...
fetch("http://jsonplaceholder.typicode.com/posts")
  ...
  //   delete data[key];
  //   // console.log(data);
  //   return console.log("deleted key object", data);
  //   // return data.json();
  // })
  .then((objData) => {
    ...
    for (let i = 0; i < 10; i++) {
      tableData += `<tr>`;
      tableData += `<td><input type=checkbox>${objData[i].id}</td>`;
      tableData += `<td>${objData[i].userId}</td>`;
      // tableData += `<td>${objData[i].id}</td>`;
      tableData += `<td>${objData[i].title}</td>`;
      tableData += `</tr>`;
      document.getElementById("table").innerHTML = tableData;
    }

    // function delete data on display
    ...
    //delete response
    let deletedData = objData;
    let key = 3;
    delete deletedData[key];
    const newObjData = deletedData;
    console.log("deleted key object", newObjData);
  })
